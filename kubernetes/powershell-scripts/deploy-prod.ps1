...
kubectl create secret generic rabbitmq-credentials --from-literal=SPRING_RABBITMQ_USERNAME=rabbit-user-prod --from-literal=SPRING_RABBITMQ_PASSWORD=rabbit-pwd-prod --save-config
kubectl create secret generic rabbitmq-zipkin-credentials --from-literal=RABBIT_USER=rabbit-user-prod --from-literal=RABBIT_PASSWORD=rabbit-pwd-prod --save-config
kubectl create secret generic mongodb-credentials --from-literal=SPRING_DATA_MONGODB_AUTHENTICATION_DATABASE=admin --from-literal=SPRING_DATA_MONGODB_USERNAME=mongodb-user-prod --from-literal=SPRING_DATA_MONGODB_PASSWORD=mongodb-pwd-prod --save-config
kubectl create secret generic mysql-credentials --from-literal=SPRING_DATASOURCE_USERNAME=mysql-user-prod --from-literal=SPRING_DATASOURCE_PASSWORD=mysql-pwd-prod --save-config
